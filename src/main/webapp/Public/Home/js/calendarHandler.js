(function(window){
    ...
    var UI={
        //drawDots:function(date,num){
//			var d = $("div[day_v='"+date+"']");
//			if(d.length>0){
//				var t = d.children();
//				if(num > 0){
//					$(t[1]).addClass("hasWorkDot");
//				}else{
//					$(t[1]).removeClass("hasWorkDot");
//				}
//			}
//		},
        drawSch:function(schList){
            ...
            if(schList.length>0){
                for(var i=0;i<2 && i<schList.length;i++){
                    var showTxt=cutByRealLength(schList[i].text,8);
                    if(showTxt.length<schList[i].text.length){
                        showTxt+="...";
                    }
                    $("#work_item"+i+">.work_item_dot").show();
                    $("#work_item"+i+">.work_item_content").html(showTxt).attr("title",schList[i].text);
                }
            }
        }
    }
